...
`Spawn/` вызывает функцию (через `lu_spawn`). Аргумент должен быть именем функции.

### Deprecated (Server/Route/Bcast)

Эти v1.x сетевые примитивы генерируют stub-код без реальной функциональности. Использование вызывает deprecation warning. Не используйте в новом коде — будут удалены в v4.0.
