...
### Не работает (явно не поддерживается)

Эти конструкции компилируются, но генерируют **невалидный C** — использование приведёт к segfault:

- **`On/name callback`** — передаёт строку как callback, но `lu_event_on` ожидает `void(*)(void*)`
- **`Spawn/expr`** — передаёт int как function pointer, но `lu_spawn` ожидает `void(*)(void)`

Не используйте их. Если нужны колбэки — передавайте C-функции через FFI (будущая работа).

### Экспериментально (не тестировалось)

- `Server #qN cor/{1,2,3}` — декларация сервера (генерирует `lu_server_t`)
- `Snd{payload}`, `Bcast{payload}` — сетевая отправка
- `Route/ dst via gateway` — маршрутизация
- `inq;` — запрос
- `cor/{1,2,3}` — ключи корреляции
- `Def:anw(expr)/Lin` — связанные ответы (legacy)

Эти примитивы генерируют вызовы к рантайму, но не имеют тестов и могут содержать баги того же класса, что `On/` и `Spawn/`.
