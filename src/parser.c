...
/* Сырой текст компилируемого файла — нужен asm-блоку, чтобы сохранить
   точный текст инструкций (лексер рвёт $2, %rax на отдельные токены).
   Устанавливается в main.c до parse(). */
const char *g_lu_src = NULL;

/* Вырезать сырой текст asm-блока: строки sl..el, колонки sc..ec. */
static char *asm_extract_raw(int sl, int sc, int el, int ec) {
    if (!g_lu_src) return NULL;
    static char buf[8192];
    int blen = 0;
    const char *p = g_lu_src;
    int ln = 1;
    while (*p && ln < sl) { if (*p == '\n') ln++; p++; }
    if (ln < sl && *p) return NULL;
    for (int i = 1; i < sc - 1 && *p && *p != '\n'; i++, p++) {}
    while (*p && ln <= el) {
        const char *eol = strchr(p, '\n');
        int linelen = eol ? (int)(eol - p) : (int)strlen(p);
        int from = (ln == sl) ? (sc - 1) : 0;
        int to   = (ln == el) ? (ec - 1) : linelen;
        if (from < 0) from = 0;
        if (to > linelen) to = linelen;
        for (int i = from; i < to && blen < (int)sizeof(buf) - 8; i++)
            buf[blen++] = p[i];
...
static ASTNode *parse_fstring(const char *content, int line) {
    ...
    while (i < len) {
        /* {{ и }} — экранированные скобки (Python-семантика) */
...
static ASTNode *parse_primary(Parser *p) {
    ...
    /* Python-стиль литералы: True / False / None */
    if (t->type == TOK_IDENT && t->value &&
        (!strcmp(t->value, "True") || !strcmp(t->value, "False"))) {
        ASTNode *n = node_new(NODE_LITERAL_BOOL, line);
        n->bval = !strcmp(t->value, "True");
        consume(p); return n;
    }
    if (t->type == TOK_IDENT && t->value && !strcmp(t->value, "None")) {
        ASTNode *n = node_new(NODE_LITERAL_INT, line);
        n->ival = 0;
        n->sval = lu_strdup("null");   /* тот же маркер, что у null */
        consume(p); return n;
    }
...
/* parse_unary: Python-стиль 'not' поверх primary/postfix-цепочки.
   Определён после parse_primary; рекурсия даёт правильную правую
   ассоциативность цепочек not/not/x. */
static ASTNode *parse_unary(Parser *p) {
    Token *t = cur(p);
    if (t->type == TOK_IDENT && t->value && !strcmp(t->value, "not")) {
        consume(p);
        ASTNode *n = node_new(NODE_EXPR_UNOP, t->line);
        n->op = lu_strdup("!");
        node_list_add(&n->children, parse_unary(p));
        return n;
    }
...
static ASTNode *parse_binop(Parser *p, int min_prec) {
    ...
    while (true) {
        ...
        /* Словесные операторы Python-стиля */
        const char *word_op = NULL;
        if (cur(p)->type == TOK_IDENT && cur(p)->value) {
            if (!strcmp(cur(p)->value, "and")) { prec = 3; word_op = "&&"; }
            else if (!strcmp(cur(p)->value, "or")) { prec = 2; word_op = "||"; }
            else if (!strcmp(cur(p)->value, "in")) { prec = 8; word_op = "in"; }
        }
        if (prec < min_prec) break;
        Token *op = consume(p);
        ASTNode *right = parse_binop(p, prec + 1);
        ASTNode *n = node_new(NODE_EXPR_BINOP, op->line);
        n->op = lu_strdup(word_op ? word_op : op->value);
...
static ASTNode *parse_statement(Parser *p) {
    ...
    case TOK_IDENT: {
        ...
        /* asm { ... } — inline-ассемблер (GCC basic asm, AT&T).
           Текст сохраняется ДОСЛОВНО из исходника (лексер рвёт $2 и %rax),
           строки склеиваются через \n\t в один __asm__ volatile("..."). */
        if (!strcmp(v, "asm")) {
            consume(p);
            if (!match(p, TOK_LBRACE)) {
                lu_error(line, "asm: expected '{'");
                g_parse_error_count++;
                return node_new(NODE_LITERAL_INT, line);
            }
            int sl = cur(p)->line, sc = cur(p)->col;
            if (check(p, TOK_NEWLINE)) sc = cur(p)->col + 1;
            while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF))
                consume(p);
            int el = cur(p)->line, ec = cur(p)->col;
            expect(p, TOK_RBRACE, "asm block");
            match(p, TOK_NEWLINE);
            ASTNode *n = node_new(NODE_ASM_STMT, line);
            char *raw = (sl > 0 && el >= sl) ? asm_extract_raw(sl, sc, el, ec)
                                             : NULL;
            n->sval = raw ? raw : lu_strdup("");
            return n;
        }
