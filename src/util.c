...
/* Global error counter — incremented by lu_error and by expect() in the
   parser when it recovers from a syntax error. main() checks this and
   returns exit code 1 if > 0, so CI/scripts can detect failures. */
int g_parse_error_count = 0;
