...
typedef enum {
    ...
    NODE_ASM_STMT       /* asm { ... } — inline-блок, текст уходит в __asm__ */
} NodeKind;
...
typedef struct {
    ...
    /* user defined def main() — renamed to lu_user_main */
    bool has_user_main;
    /* программа содержит asm-блоки → глобали эмитятся volatile */
    bool has_asm;
    /* программа использует tensor_* → эмитить тензорный рантайм */
    bool need_tensor;
} Codegen;
